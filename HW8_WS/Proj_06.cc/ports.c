...
//    P5DIR |= LCD_BACKLITE; // Set backlight pin as output
//    P5OUT &= ~LCD_BACKLITE; // Start with the backlight off
//
//    P3DIR |= TEST_PROBE; // Set probe pin as output
//    P3OUT &= ~TEST_PROBE; // Set it low initially
//
//    P1DIR &= ~(BIT0 | BIT1); // Configure switches as inputs
//    P1REN |= (BIT0 | BIT1);  // Enable pull-up/down resistors
//    P1OUT |= (BIT0 | BIT1);  // Enable pull-ups
}
...
// void Redo_Port3(void){
//     switch (PORT_STATE){   // This is a conditional switch statement based off of an option chosen through switches
//         case USE_GPIO:
//             P3SEL0 &= ~SMCLK_OUT;                               // Set second bit to Low / '0' (GPIO)
//             P3SEL1 &= ~SMCLK_OUT;                               // Set first bit to Low / '0' (GPIO)
...
void Init_Port6(){
    ...
    P6SEL0 &= ~GRN_LED ; // P6_5  operation
    P6SEL1 &= ~GRN_LED ; // P6_5  operation
