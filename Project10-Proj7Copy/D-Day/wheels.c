...
void tracking_movement(void){
    ...
    double CircleDuration = 1.0; // Modify PauseSeconds to change when Circle continues tracking
...
void end_state(void){
    ...
    case 0:
        ...
        end_counter = 0;
        endMove++;
    case 1: // SPIN 90 CLOCKWISE AGAIN
        strcpy(display_line[3],"90 SPIN #2");
        display_changed = TRUE;
        LEFT_FORWARD_SPEED = LEFTARCHFWD;
        RIGHT_REVERSE_SPEED = RIGHTARCHFWD;
        if(end_counter > 20*sec_spin){
            Off_Case();
            end_counter = 0;
            endMove++;
        }
        break;
    case 2: // FORWARD FINAL
        strcpy(display_line[3],"FORWARD #3");
        display_changed = TRUE;
        LEFT_FORWARD_SPEED = LSLOWCIRCLE; //LEFTARCHFWD;
        RIGHT_FORWARD_SPEED = RSLOWCIRCLE; //RIGHTARCHFWD;
        if(end_counter > 20*sec_fwd3){
            Off_Case();
            end_counter = 0;
            endMove++;
            endFlag = 0;
